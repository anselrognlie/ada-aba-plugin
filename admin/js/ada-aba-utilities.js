(function ($) {
  ...
  const showQueryResult = async () => {
    const query = $('#ada-aba-execute-query-query').val();
    try {
  ...
  const wireExecuteQueryActions = function () {
    ...
    $('#ada-aba-execute-query-query').on('keydown', async function (e) {
      // run the query if the user presses cmd+enter
      if (e.code === "Enter" && e.metaKey && !e.shiftKey && !e.ctrlKey && !e.altKey) {
        e.preventDefault();
        await showQueryResult();
      }
    });
